...
다형성
- 만일 다형성으로 자식을 참조하는지 궁금하다면 instanceof 를 사용하여 확인한다 ( 오른쪽대상의 자식타입을 왼쪽에서 참조하는경우에도 true)
- * 오른쪽 타입에 왼쪽에 있는 인스턴스의 타입이 들어갈수있는지 대입해보면된다
-다형성으로 생성된 객체는 오버라이딩된 메서드부터 타겟팅한다


* 상속을 해준 부모가 ex)interface 자식을 참조하여 인스턴스를 생성할수있다 
-ex) Monster num1 = new Gozilla();
-   num1.sound(); = 꾸에엥
- num 1 = new KingKong(); = 우라랏
- 이런식으로 하는이유는 다형성때문 자식으로 객체를 참조함으로써 메서드를 유동성있게 사용할수있다
- 만일 Gozilla 나 Kingkong으로 한정적인 객체로 참조하면 자신의 메서드밖에 사용을 못하기때문


* 객체 지향 프로그래밍이란 객체들의 모임으로 파악하고자하는것
* 각각의 객체는 메세지를 주고받고 데이터를 처리할수있음\
- 역할과 구현을 명확히 분리해야함
- 